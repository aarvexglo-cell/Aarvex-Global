...
(function () {
  ...
  // NOT in the native app: there Capgo owns the bundle/updates, and a SW on the
  // app's https://localhost origin would fight it AND could serve stale cached JS
  // (e.g. an old captcha build) — so this is gated by a robust, load-order-safe
  // check: the app bundle always runs at localhost, real web never does.
  var _isNativeApp = (window.axIsNativeApp && window.axIsNativeApp()) || window.AX_NATIVE === true ||
    location.hostname === 'localhost' || location.hostname === '127.0.0.1';
  if ('serviceWorker' in navigator && location.protocol.startsWith('http') && !_isNativeApp) {
