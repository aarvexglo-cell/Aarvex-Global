...
function setOrderType(type) {
  orderType = type;
  const isL = type === 'limit', isS = type === 'sample', isB = type === 'bulk';
  const cardL = document.getElementById('typeCardLimit');
  const cardS = document.getElementById('typeCardSample');
  const cardB = document.getElementById('typeCardBulk');
  if (cardL) { cardL.classList.toggle('active', isL); cardL.setAttribute('aria-selected', isL); }
  if (cardS) { cardS.classList.toggle('active', isS); cardS.setAttribute('aria-selected', isS); }
  if (cardB) { cardB.classList.toggle('active', isB); cardB.setAttribute('aria-selected', isB); }
  // Re-tint the whole form (--order-accent): cyan Limit, green Sample, orange Bulk.
  const step = document.getElementById('orderStep2');
  if (step) {
    step.classList.toggle('order-theme-limit', isL);
    step.classList.toggle('order-theme-sample', isS);
    step.classList.toggle('order-theme-bulk', isB);
  }
  const limitBox = document.getElementById('qtyLimitBox');
  if (limitBox) limitBox.style.display = isL ? '' : 'none';
  document.getElementById('qtySampleBox').style.display = isS ? '' : 'none';
  document.getElementById('qtyBulkBox').style.display   = isB ? '' : 'none';
  if (isB) refreshStockFractions();
  if (isL) axLimitQtyInput();
}

/* Limit order: live "X kg available · ₹total" hint, and hard-cap the typed
   quantity at the product's free stock so the user can't over-order. */
function axLimitQtyInput() {
  if (!selectedProduct) return;
  const stock = parseFloat(selectedProduct.free_stock_kg != null ? selectedProduct.free_stock_kg : selectedProduct.available_stock_kg) || 0;
  const input = document.getElementById('o_qty_limit');
  const hint = document.getElementById('limitStockHint');
  if (input) input.max = stock || '';
  let v = parseFloat(input && input.value) || 0;
  if (stock > 0 && v > stock) { v = stock; if (input) input.value = stock; }
  if (hint) {
    if (!stock) {
      hint.innerHTML = '<i class="fa-solid fa-circle-exclamation"></i> Out of stock for new orders';
    } else {
      const price = parseFloat(selectedProduct.price_per_kg) || 0;
      const amt = (v && price) ? ' · ₹' + Math.round(v * price) : '';
      hint.innerHTML = '<i class="fa-solid fa-boxes-stacked"></i> ' + Math.round(stock) + ' kg available' + amt;
    }
  }
}
...
async function submitOrder() {
  ...
  // Pay Online / order APIs need valid auth — the 30-day session token OR a
  // fresh Google token. Only block when NEITHER is usable (axAuthExpired).
  if (typeof axAuthExpired === 'function' && axAuthExpired()) {
  ...
  if (orderType === 'bulk') {
    ...
    const stock = parseFloat(selectedProduct.free_stock_kg != null ? selectedProduct.free_stock_kg : selectedProduct.available_stock_kg) || 0;
    qty = parseFloat(gv('o_qty_limit')) || 0;
    if (qty <= 0) {
      document.getElementById('orderErrorMsg').textContent = 'Please enter how many kg you want to order.';
      errBox.classList.add('show'); return;
    }
    if (stock > 0 && qty > stock) {
      document.getElementById('orderErrorMsg').textContent = 'Only ' + Math.round(stock) + ' kg available — please lower the quantity.';
      errBox.classList.add('show'); return;
    }
  } else {
  ...
  // Native app: Cloudflare Turnstile can't load in the WebView (capacitor://localhost
  // origin), so we send the 'native-app' marker. The backend accepts it ONLY for an
  // authenticated request (unforgeable HMAC session token), so this stays bot-safe.
  const recaptchaToken = (window.axIsNativeApp && window.axIsNativeApp()) ? 'native-app' : getRecaptchaToken('orderRecaptcha');
  if (!recaptchaToken) {
  ...
  try {
    const _authToken = (window.axGetAuthToken?window.axGetAuthToken():localStorage.getItem('ax_google_token'));
    ...
    if (res.ok && data.success) {
      ...
      if (sumEl && selectedProduct) {
        sumEl.innerHTML =
          ...
          '<div class="ss-cell"><span>Quantity</span><b>' + (Math.round(qty * 100) / 100) + ' kg · ' + (orderType === 'bulk' ? 'Bulk' : orderType === 'limit' ? 'Limit' : 'Sample') + '</b></div>' +
...
function resetOrder() {
  ...
  selectedProduct = null; orderType = 'limit'; orderPurchaseFraction = null;
  document.getElementById('orderStep1').style.display = '';
  document.getElementById('orderStep2').style.display = 'none';
  document.getElementById('orderStep3').style.display = 'none';
  setOrderProgressStep(1);
  sv('o_qty', ''); sv('o_qty_limit', ''); setOrderType('limit');
}
...
async function submitSell() {
  ...
  if (missing.length) {
    ...
    errBox.classList.add('show');
    if (typeof ax26Shake === 'function') ax26Shake(errBox);
    return;
  }
  const recaptchaToken = (window.axIsNativeApp && window.axIsNativeApp()) ? 'native-app' : getRecaptchaToken('sellRecaptcha');
  if (!recaptchaToken) {
    showToast('Please complete the captcha verification', 'error');
    return;
  }
  ...
  try {
    ...
    if (res.ok && data.success) {
      ...
      const successCard = document.querySelector('#sellSuccess .success-card');
      if (typeof axConfettiBurst === 'function') axConfettiBurst(successCard || document.getElementById('sellSuccess'));
    } else {
...
async function trackOrder() {
  ...
  try {
    const res = await fetch(LAMBDA_URL + '/track?arn=' + encodeURIComponent(arn), {
      headers: { Authorization: 'Bearer ' + axGetAuthToken(), Accept: 'application/json' },
    });
    const data = typeof safeFetchJson === 'function' ? await safeFetchJson(res) : await res.json();
...
async function axLoadAccount() {
  ...
  if (!el) return;
  el.innerHTML = '<div class="ax-account-loading ax26-skel-wrap" aria-busy="true" aria-label="Loading account">' +
    '<div class="ax26-skel-card skeleton"></div>' +
    '<div class="ax26-skel-card skeleton"></div>' +
    '<div class="ax26-skel-card tall skeleton"></div>' +
  '</div>';
  let data = {};
  ...
  if ((totCod || totOnline) && typeof axDonut === 'function') {
    html += '<div class="ax-acct-card"><div class="ax-acct-card-head"><i class="fa-solid fa-chart-pie"></i> Payment mix</div>' +
      axDonut([{ label: 'COD', value: totCod, color: '#A78BFA' }, { label: 'Online', value: totOnline, color: '#34D399' }],
  ...
  if (!html) {
    html = (typeof ax26EmptyHtml === 'function')
      ? ax26EmptyHtml({
          icon: 'wallet',
          title: 'No account activity yet',
          message: 'Your earnings, payouts and orders will appear here once you buy, sell, or deliver.',
          ctaHtml: '<button type="button" class="btn-primary" style="margin-top:16px" onclick="switchPanel(\'trade\')"><i class="fa-solid fa-bag-shopping"></i> Browse Trade</button>'
        })
      : ((typeof axEmptyState === 'function')
