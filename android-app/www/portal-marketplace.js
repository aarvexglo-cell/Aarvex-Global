...
function mpAuthHeaders() {
  ...
  // Prefer the long-lived 30-day session token; fall back to the Google ID token
  // (bootstrap request + back-compat).
  const token = (typeof window.axGetAuthToken === 'function' ? window.axGetAuthToken() : '')
    || localStorage.getItem('ax_session_token') || localStorage.getItem('ax_google_token');
...
async function mpApi(path, opts = {}) {
  ...
  if (res.status === 401) {
    ...
    // A 401 means the bearer we sent was rejected — if that was a session token,
    // drop it so the retry re-authenticates via Google and mints a fresh one.
    try { localStorage.removeItem('ax_session_token'); } catch (e) {}
...
function renderDashboardCta(data) {
  ...
  if (showShop && !mpShop) {
    html += `
      <div class="dash-cta-grid" style="grid-template-columns:1fr;gap:var(--s3);margin-bottom:var(--s4)">
        <div class="cta-card shop-cta" onclick="switchPanel('shop')">
...
/* Panel data is loaded on first use instead of during sign-in. The flags are
   deliberately separate from the fetch functions so existing refresh actions
   can still call mpLoadNotifications/mpLoadShopProducts directly. */
const _mpLazyPanels = Object.create(null);
function mpEnsurePanelLoaded(panelId) {
  if (panelId === 'sell' || panelId === 'order') panelId = 'trade';
  if (_mpLazyPanels[panelId]) return;
  _mpLazyPanels[panelId] = true;
  if (panelId === 'dashboard') {
    mpLoadBanner();
    mpLoadKycStatus();
  } else if (panelId === 'notifications') {
    mpLoadNotifications();
  } else if (panelId === 'shop') {
    mpLoadShopProducts();
    mpLoadShopFeedback();
  } else if (panelId === 'profile') {
    mpLoadKycStatus();
    mpGenerateCaptcha();
  } else if (panelId === 'delivery' && typeof mpLoadDeliveryDashboard === 'function') {
    mpLoadDeliveryDashboard();
  } else if (panelId === 'trade') {
    mpLoadBanner('trade', 'tradeBannerSlot');
  }
}
window.mpEnsurePanelLoaded = mpEnsurePanelLoaded;
...
async function mpSubmitKyc() {
  ...
  // Native app: Turnstile can't load in the WebView; send 'native-app' (backend
  // accepts it only for the authenticated caller). Web keeps the real captcha.
  const recaptchaToken = (window.axIsNativeApp && window.axIsNativeApp())
    ? 'native-app'
    : (typeof getRecaptchaToken === 'function' ? getRecaptchaToken('kycRecaptcha') : '');
  ...
  if (data.success) {
    try { if (typeof kycKey === 'function') localStorage.removeItem(kycKey()); } catch (e) { }
...
/* One person-row inside a claim card: role + name on the left, the three
   ...
   right. */
/* App's own messenger glyph (same mark as the top-nav Messages button) so the
   shop contact rows use the brand chat icon instead of a generic FA bubble. */
function axMsgIconSvg() {
  return '<svg viewBox="0 0 24 24" width="15" height="15" aria-hidden="true" focusable="false" style="display:block">' +
    '<path fill="currentColor" d="M19.5 8C20.8807 8 22 6.88071 22 5.5C22 4.11929 20.8807 3 19.5 3C18.1193 3 17 4.11929 17 5.5C17 6.88071 18.1193 8 19.5 8Z"/>' +
    '<path fill="currentColor" d="M20.72 9.31C20.02 9.53 19.25 9.57 18.45 9.37C17.11 9.02 16.02 7.95 15.65 6.61C15.47 5.96 15.46 5.32 15.57 4.74C15.7 4.1 15.25 3.5 14.61 3.5H7C4 3.5 2 5 2 8.5V15.5C2 19 4 20.5 7 20.5H17C20 20.5 22 19 22 15.5V10.26C22 9.6 21.36 9.1 20.72 9.31ZM15.52 11.15L14.34 12.09C13.68 12.62 12.84 12.88 12 12.88C11.16 12.88 10.31 12.62 9.66 12.09L6.53 9.59C6.21 9.33 6.16 8.85 6.41 8.53C6.67 8.21 7.14 8.15 7.46 8.41L10.59 10.91C11.35 11.52 12.64 11.52 13.4 10.91L14.58 9.97C14.9 9.71 15.38 9.76 15.63 10.09C15.89 10.41 15.84 10.89 15.52 11.15Z"/>' +
    '</svg>';
}
window.axMsgIconSvg = axMsgIconSvg;
...
function axContactRow(role, icon, name, phone, waMsg) {
  return '<div class="shop-claim-row">' +
    ...
    (phone
      ...
      '<a class="sc-btn sc-sms" href="sms:' + escS(phone) + '" title="Message" aria-label="SMS">' + axMsgIconSvg() + '</a>' +
...
async function mpLoadAnalytics() {
  ...
  const s = d.sales || {};
  // Live preview on the collapsed "Shop Performance" summary.
  const perfHint = document.getElementById('axShopPerfHint');
  if (perfHint) perfHint.textContent = inr(s.revenue) + ' · ' + (s.orders || 0) + ' orders';
  ...
  if ((s.cod || s.online) && typeof axDonut === 'function') {
    html += '<div class="ax-analytics-chart">' + axDonut([
      ...
      { label: 'Online', value: s.online || 0, color: '#0891B2' },
...
async function mpLoadShopProducts() {
  ...
  // Live preview on the collapsed "Shop Pickup Address" summary.
  const pickupHint = document.getElementById('axShopPickupHint');
  if (pickupHint) {
    const up = (typeof userProfile !== 'undefined' && userProfile) ? userProfile : {};
    pickupHint.textContent = (up.shop_lat && up.shop_lng)
      ? '✓ ' + (up.shop_city || 'Pinned')
      : 'Not set — using personal address';
  }
  ...
  // Split products into category buckets (grid) + a demand list (live orders).
  const _cats = {};
  const _demand = [];
  mpProducts.forEach(function (p) {
    const key = (p.category_name || 'Other').trim() || 'Other';
    (_cats[key] = _cats[key] || []).push(p);
    if ((p.claimed_orders || []).length) _demand.push(p);
  });

  // ── C1: ACTIVE ORDERS / DEMAND panel (highlighted — buyer + partner + contacts) ──
  const demandWrap = document.getElementById('shopDemandWrap');
  const demandList = document.getElementById('shopDemandList');
  if (demandList) {
    // Compact grid: image + name + total kg + order-count badge. Full contacts,
    // ARN and Track-live open in a tap sheet (axShopOrderDetail) — keeps this
    // panel short even with many active orders.
    demandList.innerHTML = _demand.map(function (p) {
      const media = p.image_url
        ? '<img class="ax-dmd-gimg" src="' + escS(p.image_url) + '" alt="" loading="lazy" onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'grid\'"><div class="ax-dmd-gimg ax-dmd-ph" style="display:none"><i class="fa-solid fa-box-open"></i></div>'
        : '<div class="ax-dmd-gimg ax-dmd-ph"><i class="fa-solid fa-box-open"></i></div>';
      const co = p.claimed_orders || [];
      const count = co.length;
      const totalKg = co.reduce(function (s, c) { return s + (parseFloat(c.ordered_kg || c.reserved_kg) || 0); }, 0);
      return '<button type="button" class="ax-dmd-tile" onclick="axShopOrderDetail(\'' + p.product_id + '\')">' +
        media +
        '<span class="ax-dmd-gcount">' + count + '</span>' +
        '<div class="ax-dmd-gbody">' +
        '<div class="ax-dmd-gname">' + escS(p.product_name) + '</div>' +
        '<div class="ax-dmd-gmeta">' + (totalKg ? Math.round(totalKg) + ' kg · ' : '') + count + ' order' + (count > 1 ? 's' : '') + '</div>' +
        '</div>' +
        '</button>';
    }).join('');
  }
  if (demandWrap) demandWrap.style.display = _demand.length ? '' : 'none';

  // ── C2: YOUR PRODUCTS — compact category-wise grid (all products, normal size) ──
  ...
  const catNames = Object.keys(_cats).sort();
  grid.className = 'shop-cat-wrap';
  grid.innerHTML = catNames.map(function (cat) {
    const cards = _cats[cat].map(function (p) {
      const avail = parseFloat(p.available_stock_kg);
      const paused = !!p.is_paused;
      const claims = (p.claimed_orders || []).length;
      const isSoldOut = p.lot_status === 'sold_out' || (!isNaN(avail) && avail <= 0 && !claims);
      const isReserved = p.lot_status === 'reserved' || p.lot_status === 'sold';
      const chip = isSoldOut ? '<span class="sp-chip sp-sold">Sold</span>'
        : isReserved ? '<span class="sp-chip sp-sold">Out</span>'
          : paused ? '<span class="sp-chip sp-paused">Paused</span>'
            : '<span class="sp-chip sp-live"><i class="fa-solid fa-circle"></i> Live</span>';
      const media = p.image_url
        ? '<img src="' + escS(p.image_url) + '" alt="" loading="lazy" onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'grid\'"><div class="sp-grid-ph" style="display:none"><i class="fa-solid fa-box-open"></i></div>'
        : '<div class="sp-grid-ph"><i class="fa-solid fa-box-open"></i></div>';
      const price = p.price_per_kg ? '₹' + p.price_per_kg + '/kg' : (p.lot_price ? '₹' + p.lot_price : '');
      const stock = !isNaN(avail) ? Math.round(avail) + ' kg' : '';
      return '<div class="sp-grid-card" id="shop-prod-' + escS(p.product_id) + '">' +
        '<div class="sp-grid-media" onclick="mpOpenEditProduct(\'' + p.product_id + '\',\'' + p.category_id + '\')">' + media + chip +
        (claims ? '<span class="sp-grid-demand">' + claims + ' order' + (claims > 1 ? 's' : '') + '</span>' : '') +
        '</div>' +
        '<button type="button" class="sp-grid-kebab" aria-label="Options" onclick="axShopProductMenu(\'' + p.product_id + '\',\'' + p.category_id + '\')"><i class="fa-solid fa-ellipsis-vertical"></i></button>' +
        '<div class="sp-grid-body" onclick="mpOpenEditProduct(\'' + p.product_id + '\',\'' + p.category_id + '\')">' +
        '<div class="sp-grid-name">' + escS(p.product_name) + '</div>' +
        '<div class="sp-grid-price">' + escS(price) + (stock ? ' <span class="sp-grid-stock">· ' + stock + '</span>' : '') + '</div>' +
        '</div>' +
        '</div>';
    }).join('');
    return '<div class="shop-cat-label">' + escS(cat) + ' <span class="shop-cat-count">' + _cats[cat].length + '</span></div>' +
      '<div class="shop-cat-grid">' + cards + '</div>';
  }).join('');
}

/* Action sheet for a grid product card's ⋮ (kebab) — Modify / Pause / Delete. */
function axShopProductMenu(pid, cid) {
  const p = (typeof mpProducts !== 'undefined' ? mpProducts : []).find(function (x) { return x.product_id === pid; });
  if (!p) return;
  const paused = !!p.is_paused;
  const avail = parseFloat(p.available_stock_kg);
  const isSoldOut = p.lot_status === 'sold_out' || (!isNaN(avail) && avail <= 0 && !(p.claimed_orders || []).length);
  const isReserved = p.lot_status === 'reserved' || p.lot_status === 'sold';
  const rm = "document.getElementById('axShopProdSheet').remove();";
  let ov = document.getElementById('axShopProdSheet');
  if (ov) ov.remove();
  ov = document.createElement('div');
  ov.id = 'axShopProdSheet';
  ov.className = 'ax-actionsheet';
  ov.innerHTML = '<div class="ax-actionsheet-card ax-prodmenu">' +
    '<div class="ax-prodmenu-title">' + escS(p.product_name) + '</div>' +
    '<button type="button" class="ax-prodmenu-item" onclick="' + rm + "mpOpenEditProduct('" + pid + "','" + cid + "')\"><i class=\"fa-solid fa-pen-to-square\"></i> Modify</button>" +
    (isReserved ? '' : (isSoldOut ? '' :
      '<button type="button" class="ax-prodmenu-item" onclick="' + rm + "mpToggleProductPause('" + pid + "','" + cid + "'," + (!paused) + ')"><i class="fa-solid fa-' + (paused ? 'play' : 'pause') + '"></i> ' + (paused ? 'Activate' : 'Pause') + '</button>')) +
    '<button type="button" class="ax-prodmenu-item ax-prodmenu-danger" onclick="' + rm + "mpDeleteProduct('" + pid + "','" + cid + "')\"><i class=\"fa-regular fa-trash-can\"></i> Delete</button>" +
    '<button type="button" class="ax-prodmenu-cancel" onclick="' + rm + '">Cancel</button>' +
    '</div>';
  ov.addEventListener('click', function (e) { if (e.target === ov) ov.remove(); });
  document.body.appendChild(ov);
}
window.axShopProductMenu = axShopProductMenu;

/* Shop control bar — accordion over the 3 header sections (Manage / Pickup /
   Performance). Clicking the open tab collapses it; otherwise open that one and
   close the rest. Products + Active Orders stay visible below regardless. */
function axShopSection(which) {
  const map = {
    manage: ['shopTabManage', 'shopSecManage'],
    pickup: ['shopTabPickup', 'shopSecPickup'],
    perf:   ['shopTabPerf',   'shopSecPerf'],
  };
  const cur = window._axShopSecOpen === undefined ? 'manage' : window._axShopSecOpen;
  const next = (cur === which) ? null : which;
  window._axShopSecOpen = next;
  Object.keys(map).forEach(function (k) {
    const tab = document.getElementById(map[k][0]);
    const sec = document.getElementById(map[k][1]);
    const on = (k === next);
    if (tab) { tab.classList.toggle('active', on); tab.setAttribute('aria-expanded', on ? 'true' : 'false'); }
    if (sec) { sec.style.display = on ? '' : 'none'; }
  });
}
window.axShopSection = axShopSection;

/* Active-order detail sheet — opened by tapping a demand grid tile. Shows every
   claimed order's status, kg, ARN (copy), buyer + delivery-partner contacts, and
   a Track-live button that opens the live map (shop + delivery-boy locations). */
function axShopOrderDetail(pid) {
  const list = (typeof mpProducts !== 'undefined') ? mpProducts : [];
  const p = list.find(function (x) { return x.product_id === pid; });
  if (!p) return;
  const shopName = (typeof mpShop !== 'undefined' && mpShop && mpShop.shop_name) || 'My Shop';
  const claimStatusLabel = {
    PENDING_DELIVERY: 'Order placed', PENDING_APPROVAL: 'Awaiting approval',
    DELIVERY_ASSIGNED: 'Claimed', IN_TRANSIT: 'Out for delivery', NEAR_DESTINATION: 'Arriving soon',
  };
  const co = p.claimed_orders || [];
  const rm = "document.getElementById('axShopOrderSheet').remove();";
  const orders = co.map(function (c) {
    const label = claimStatusLabel[c.status] || c.status || 'Open';
    const kg = c.ordered_kg || c.reserved_kg;
    const st = c.status || '';
    const stCls = /IN_TRANSIT|NEAR/.test(st) ? 'sp-transit' : (/ASSIGNED/.test(st) ? 'sp-claimed' : 'sp-placed');
    const waB = 'Namaste ' + (c.buyer_name || '') + '! Main ' + shopName + ' (Aarvex Global) se aapke order ' + (c.arn || '') + ' — ' + (p.product_name || '') + ' — ke baare mein sampark kar raha/rahi hun.';
    const waP = 'Namaste ' + (c.delivery_partner_name || '') + '! ' + shopName + ' (Aarvex Global) se — order ' + (c.arn || '') + ' (' + (p.product_name || '') + ') ki delivery ke baare mein.';
    return '<div class="ax-dmd-order">' +
      '<div class="ax-dmd-order-head"><span class="sp-chip ' + stCls + '">' + escS(label) + '</span>' +
      (kg ? '<span class="ax-dmd-kg">' + escS(kg) + ' kg</span>' : '') +
      ...
      '</div>' +
      axContactRow('Buyer', 'fa-user', c.buyer_name || 'Buyer', c.buyer_mobile, waB) +
      (c.delivery_partner_name || c.delivery_partner_mobile
        ? axContactRow('Delivery partner', 'fa-truck-fast', c.delivery_partner_name, c.delivery_partner_mobile, waP)
        : '<div class="ax-dmd-waiting">Waiting for a delivery partner…</div>') +
      (c.arn
        ? '<button type="button" class="ax-dmd-track" onclick="' + rm + "if(typeof axOpenTrackFromNotif==='function')axOpenTrackFromNotif('" + escS(c.arn) + "')\"><i class=\"fa-solid fa-location-crosshairs\"></i> Track live — shop &amp; delivery boy location</button>"
        : '') +
      '</div>';
  }).join('');
  let ov = document.getElementById('axShopOrderSheet');
  if (ov) ov.remove();
  ov = document.createElement('div');
  ov.id = 'axShopOrderSheet';
  ov.className = 'ax-actionsheet';
  ov.innerHTML = '<div class="ax-actionsheet-card ax-dmd-sheet">' +
    '<div class="ax-dmd-sheet-head">' +
    '<div class="ax-dmd-sheet-title">' + escS(p.product_name) + '</div>' +
    '<div class="ax-dmd-sheet-sub">' + escS(p.category_name || '') + ' · ' + co.length + ' active order' + (co.length > 1 ? 's' : '') + '</div>' +
    '</div>' +
    '<div class="ax-dmd-sheet-body">' + (orders || '<div class="ax-dmd-waiting">No active orders.</div>') + '</div>' +
    '<button type="button" class="ax-prodmenu-cancel" onclick="' + rm + '">Close</button>' +
    '</div>';
  ov.addEventListener('click', function (e) { if (e.target === ov) ov.remove(); });
  document.body.appendChild(ov);
}
window.axShopOrderDetail = axShopOrderDetail;
