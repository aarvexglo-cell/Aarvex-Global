...
const PortalEnhancements = (function () {
  ...
  function renderAlertList(items) {
    ...
    list.innerHTML = items.slice(0, 12).map(function (n) {
      ...
      const isOtp = n.type === 'delivery_otp' || otp;
      return '<div class="notif-alert-item' + (n.is_read ? '' : ' unread') + (isOtp ? ' otp-item' : '') + '">' +
  ...
  async function refreshNotifications(silent) {
    ...
    if (!silent && unread > _lastUnread && _lastUnread >= 0) {
      spawnFlyingLetter();
      if (typeof showToast === 'function') showToast('You have ' + unread + ' notification' + (unread === 1 ? '' : 's'), 'info');
  ...
  function initProfileTabs() {
    ...
    if (!panel || panel.querySelector('.profile-tab-bar')) return;
    const sections = [
      { id: 'profileTabPersonal', label: 'Personal Information', icon: 'fa-user' },
      { id: 'profileTabAddress', label: 'Default Delivery Address', icon: 'fa-location-dot' },
      { id: 'profileTabBusiness', label: 'Business Information', icon: 'fa-building' },
      { id: 'profileTabKyc', label: 'KYC Verification', icon: 'fa-id-card' },
    ];
    const bar = document.createElement('div');
    bar.className = 'profile-tab-bar';
    bar.innerHTML = sections.map(function (s, i) {
      return '<button type="button" class="profile-tab-btn' + (i === 0 ? ' active' : '') + '" data-tab="' + s.id + '"><i class="fa-solid ' + s.icon + '"></i><span>' + s.label + '</span></button>';
    }).join('');
    ...
    if (formSections[0]) { formSections[0].classList.add('profile-tab-pane'); formSections[0].dataset.profileTab = 'profileTabPersonal'; }
    if (formSections[1]) { formSections[1].classList.add('profile-tab-pane'); formSections[1].dataset.profileTab = 'profileTabAddress'; formSections[1].style.display = 'none'; }
    if (formSections[2]) { formSections[2].classList.add('profile-tab-pane'); formSections[2].dataset.profileTab = 'profileTabBusiness'; formSections[2].style.display = 'none'; }
    if (saveRow) { saveRow.classList.add('profile-tab-pane'); saveRow.dataset.profileTab = 'profileTabPersonal'; }
    const kyc = document.getElementById('kycSection');
    if (kyc) { kyc.classList.add('profile-tab-pane'); kyc.dataset.profileTab = 'profileTabKyc'; kyc.style.display = 'none'; }
    bar.addEventListener('click', function (e) {
      const btn = e.target.closest('.profile-tab-btn');
      if (!btn) return;
      const tab = btn.dataset.tab;
      bar.querySelectorAll('.profile-tab-btn').forEach(function (b) { b.classList.toggle('active', b === btn); });
      panel.querySelectorAll('.profile-tab-pane').forEach(function (p) {
        p.style.display = p.dataset.profileTab === tab ? '' : 'none';
      });
      if (tab === 'profileTabKyc' && typeof mpGenerateCaptcha === 'function') mpGenerateCaptcha();
    });
  }
  ...
  function _carIconHtml(heading) {
    return '<div class="car-tracking-icon-wrap" style="width:44px;height:44px;display:flex;align-items:center;justify-content:center;">' +
      '<img src="' + CAR_TRACKING_ICON_URL + '" alt="" style="width:34px;height:auto;transform:rotate(' + (heading || 0) + 'deg);transform-origin:center center;transition:transform .6s ease-out;filter:drop-shadow(0 3px 6px rgba(13,11,8,.35));display:block;">' +
      '</div>';
  }
    ...
    ensureGoogleMaps(function (failed) {
      if (failed !== null && GOOGLE_MAPS_API_KEY && !_googleMapsAuthFailed) {
        ...
        if (driver && driver.lat) {
          ...
          if (dest.lat && dest.lng) {
            _renderRoadRoute(_trackGMap, _trackDirRef, { lat: driver.lat, lng: driver.lng }, { lat: dest.lat, lng: dest.lng }, '#2E6B41', function (leg) {
              if (leg) {
                const card = document.getElementById('trackLiveCard');
                const statsEl = card && card.querySelector('.ltc-stats');
                if (statsEl) {
                  statsEl.innerHTML =
                    '<div class="ltc-stat"><strong>' + leg.distance.text + '</strong><span>Road distance</span></div>' +
                    '<div class="ltc-stat"><strong>' + leg.duration.text + '</strong><span>ETA (live traffic)</span></div>';
                }
              }
      ...
      ensureLeaflet(function () {
        ...
        if (!_trackMap) {
          ...
          L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution: '© OpenStreetMap' }).addTo(_trackMap);
        ...
        if (dest.lat && dest.lng) {
          ...
          const destIcon = L.divIcon({
            className: 'dest-icon',
            html: '<div style="color:#D93644;font-size:32px;"><i class="fa-solid fa-location-dot"></i></div>',
            iconSize: [32, 32],
            iconAnchor: [16, 32]
          });
          L.marker([dest.lat, dest.lng], { icon: destIcon }).addTo(_trackMap).bindPopup('Destination · ' + (data.buyer_name || dest.address || dest.city || 'Importer'));
        } else {
  ...
  function fetchTrack(arn) {
    return fetch(LAMBDA_URL + '/track?arn=' + encodeURIComponent(arn), {
  ...
  function renderDeliveryActiveMapData(arn, data) {
    ...
    if (deliveryInfoCard) {
      ...
      if (deliveryDistance) deliveryDistance.textContent = data.distance_remaining_km ? data.distance_remaining_km.toFixed(1) + ' km' : '-- km';
      if (deliveryETA) deliveryETA.textContent = data.time_remaining_min ? data.time_remaining_min + ' min' : '-- min';
      if (deliveryEarnings) deliveryEarnings.textContent = data.earnings ? '₹' + data.earnings : '₹--';
    }
    ...
    renderLiveTrackCard('deliveryLiveCard', data, {
      person: data.buyer_name ? { name: data.buyer_name, phone: '' } : null,
      personLabel: 'Delivering to'
    });
    const dest = data.destination || {};
    const driver = data.driver_location;
    ...
    ensureGoogleMaps(function (failed) {
      if (failed !== null && GOOGLE_MAPS_API_KEY && !_googleMapsAuthFailed) {
        ...
        if (driver && driver.lat) {
          ...
          if (dest.lat && dest.lng) {
            _renderRoadRoute(_deliveryGMap, _deliveryDirRef, { lat: driver.lat, lng: driver.lng }, { lat: dest.lat, lng: dest.lng }, '#2563EB', function (route) {
              const bounds = new google.maps.LatLngBounds();
              bounds.extend({ lat: driver.lat, lng: driver.lng });
              bounds.extend({ lat: dest.lat, lng: dest.lng });
      ...
      ensureLeaflet(function () {
        if (!_deliveryMap) {
          ...
          L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution: '© OSM' }).addTo(_deliveryMap);
        }
        ...
        if (dest.lat && dest.lng) {
          ...
          L.marker([dest.lat, dest.lng], { icon: destIcon }).addTo(_deliveryMap).bindPopup('Destination · ' + esc(data.buyer_name || dest.address || dest.city || 'Importer'));
        }
  ...
  function toggleTrackMapFullscreen() {
    ...
    const fullscreenBtn = document.querySelector('#trackMapControlButtons button[title="Fullscreen"]');
    
    if (!mapWrap) return;
    
    const isFullscreen = mapWrap.classList.toggle('fullscreen');
    ...
    setTimeout(function() {
      ...
      if (_trackGMap) {
        google.maps.event.trigger(_trackGMap, 'resize');
      }
    }, 350);
  }
  ...
  // Delivery map fullscreen function
  window.toggleDeliveryFullscreen = function() {
    const mapWrap = document.getElementById('deliveryMapWrap');
    if (!mapWrap) return;

    _isDeliveryFullscreen = mapWrap.classList.toggle('fullscreen');

    // Invalidate map size after the CSS aspect-ratio transition settles —
    // this is the actually-used Leaflet map (Google Maps only runs if a
    // key is configured), so it must be the one that gets refreshed.
    setTimeout(function() {
